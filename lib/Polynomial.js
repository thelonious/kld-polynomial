...
Polynomial.prototype.getCubicRoots = function() {
    ...
    if ( this.getDegree() == 3 ) {
        ...
        var ZEROepsilon = Math.pow(2, this.getDegree() + 2) * Math.pow(10, -(15) + Math.floor(Math.log(Math.abs(this.coefs[0])) / Math.LN10));
...
Polynomial.prototype.getQuarticRoots = function () {
    ...
    if (n == 4) {
        ...
        var i;
        var ZEROepsilon = Math.pow(2, n + 2) * Math.pow(10, -(15) + Math.floor(Math.log(Math.abs(poly.coefs[0])) / Math.LN10));
        var Xepsilon = function () { return Math.max(Math.abs(derrt[i]) * 1e-14, ZEROepsilon); };
        
        for (i = 0; i <= nr; i++) {
            dery.push(poly.eval(derrt[i]));
        }
        //console.log('ZEROepsilon = ' + ZEROepsilon + ' a0 = ' + poly.coefs[0] + ' dery = ' + dery);
        for (i = 0; i <= nr; i++) {
        ...
        var rb = poly.bounds();
        //var rb = { minX: -Number.MAX_VALUE, maxX: Number.MAX_VALUE };

        var dx = 0.1 * (rb.maxX - rb.minX) / n;
        var guesses = [];
        var minmax = [];
        if (nr > -1) {
            ...
            if (dery[0] != 0) {
                if (sign(dery[0]) != sign(poly.eval(derrt[0] - dx) - dery[0])) {
                    ...
                    //guesses.push((rb.minX + derrt[0]) / 2);
                    minmax.push([rb.minX, derrt[0]]);
                }
            }
            else {
                //results.push(derrt[0], derrt[0]);
                guesses.push(derrt[i] - Xepsilon());
                minmax.push([derrt[i] - 2 * Xepsilon(), derrt[i]]);

                guesses.push((i == nr || dery[i + 1] == 0) ? derrt[i] + Xepsilon() : Math.min(derrt[i] + Xepsilon(), (derrt[i] + derrt[i + 1]) / 2));
                minmax.push([derrt[i], derrt[i] + 2 * Xepsilon()]);
                i++;
            }
            ...
            for (; i < nr; i++) {
                if (dery[i + 1] == 0) {
                    ...
                    i++;
                    guesses.push((i == 0 || dery[i - 1] == 0) ? derrt[i] - Xepsilon() : Math.max(derrt[i] - Xepsilon(), (derrt[i] + derrt[i - 1]) / 2));
                    minmax.push([derrt[i] - 2 * Xepsilon(), derrt[i]]);
            ...
            if (dery[nr] != 0 && sign(dery[nr]) != sign(poly.eval(derrt[nr] + dx) - dery[nr])) {
                ...
                //guesses.push((derrt[nr] + rb.maxX) / 2);
...
Polynomial.prototype.bounds_UpperReal_Fujiwara = function () {
    ...
    return {
        negX: -(max_nearmax_neg.max + max_nearmax_neg.nearmax),
        posX: max_nearmax_pos.max + max_nearmax_pos.nearmax
    };
};
